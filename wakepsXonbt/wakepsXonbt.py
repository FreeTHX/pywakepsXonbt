...
def hci_cc(adapter: int, psXbtaddr: str) -> None:
    import bluetooth._bluetooth as _bt
    ...
    try:
        ...
        # Get bytes from bluetooth address
        bapsXaddr = bytearray.fromhex(psXbtaddr.replace(":",""))
        bapsXaddr.reverse()
        # HCI CONNECT
        ...
        cmd_pkt = bytes(bapsXaddr) + bytes([ # Target BTADDR on 6 bytes reverse order,
...
def send_magic_packet(adapter,
    ...
    # Spoof the addr
    write_local_bdaddr[compid](adapterdevid, ds_addr)
    # connect to the PsX HCI CC
    hci_cc(adapterdevid, psX_addr)
